...
export default function OrbitChat() {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl border border-zinc-200 bg-white shadow-[0_30px_60px_-25px_rgba(0,0,0,0.15)] dark:border-white/10 dark:bg-[#0E0E11] dark:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)]">
      {/* Header */}
      ...
      {/* Mensagens */}
      <div ref={boxRef} className="scroll-slim h-[420px] space-y-6 overflow-y-auto px-5 py-6">
        {messages.map((m, i) => (
          <div key={i}>
            {m.role === "user" ? (
      ...
      <form onSubmit={send} className="flex items-center gap-3 border-t border-zinc-200 px-4 py-3.5 dark:border-white/[0.06]">
        ...
        <input
          ...
          placeholder={blocked ? "Disponível novamente amanhã." : awaitingDesc ? "Descreva o produto + estilo: ex.: capacete de moto preto, fundo branco" : pendingProduct ? "Responda: 1, 2, 3, fundo branco, transparente ou cenário…" : "Pergunte ou anexe a foto de um produto…"}
